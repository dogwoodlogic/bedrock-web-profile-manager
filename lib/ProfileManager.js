...
export class ProfileManager {
  ...
  async delegateEdvCapabilities({
    ...
    if(hmac) {
      delegateEdvHmacRequest.invocationTarget = hmac.kmsId;
      const keystoreId = utils.parseKeystoreId(hmac.kmsId);
    ...
    if(keyAgreementKey) {
      ...
      const keystoreId = utils.parseKeystoreId(keyAgreementKey.kmsId);
      const parentZcap = `urn:zcap:root:${encodeURIComponent(keystoreId)}`;
