...
export class AccessManager {
  ...
  async createUser({content = {}, token}) {
    ...
    const {_profileService: profileService, accountId} = profileManager;
  ...
  async updateUser({id, user, mutator, retry = true} = {}) {
    const userDoc = await this.users.update({
      id, item: user, mutator, retry
    });
  ...
  async removeUser({id} = {}) {
    ...
    const {
      profileManager: {_profileService: profileService, accountId}
    } = this;
